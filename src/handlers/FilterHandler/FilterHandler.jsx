...
export function FilterHandler() {
  ...
  const handlerSort = async (event) => {
    ...
    try {
      const { data } = await axios(
        `http://localhost:3001/books/booksort?value=${value}&organization=${organization}&page=${page}`
      );
      if (data) {
  ...
  const handlerFilter = async (event) => {
    ...
    try {
      const { data } = await axios(
        `http://localhost:3001/books/filter?author=${author}&year=${year}&gender=${gender}&page=${page}`
      );
      if (data) {
        const totalPages = Math.ceil(data.count / 4);
        dispatch(setTotalData(totalPages));
